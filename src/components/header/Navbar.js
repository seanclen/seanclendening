...
export default class Navbar extends React.Component {
  constructor(props) {
    ...
    this.playHelloAnimation = this.playHelloAnimation.bind(this)
    this.toggleNavigation = this.toggleNavigation.bind(this)

    var preloaderTimeline, landingTimeline
  }

  componentDidMount() {
    this.preloaderTimeline = anime.timeline({
      duration: 800,
      autoplay: true
    })

    this.landingTimeline = anime.timeline({
      duration: 800,
      autoplay: false
    })

    this.preloaderTimeline.add({
      targets: '#logo',
      translateY: '120px',
      translateX: '-48px',
      rotate: -90,
    ...
    this.preloaderTimeline.finished.then(() => {
      this.landingTimeline.play()
    })
  }
      ...
      <nav className={"navbar is-spaced is-transparent is-fixed-top " + isNavToggledClass} role="navigation" aria-label="main navigation">
        <div className="navbar-brand">
          <figure className={'logo-container' + isPreloaderVisibleClass + isNavToggledClass}>
            <Link className='click-box' to="/"
              onMouseEnter={this.playHelloAnimation}/>
